...
const acceptTimesheet = (req, res) => {
  ...
  db.query(getIdSql, [weekId, empId, orgId], (getErr, getRes) => {
    ...
    // 1. Check if project has hierarchy
    const checkSql = `SELECT COUNT(*) AS CNT FROM TC_PROJ_HIER_LIST WHERE PROJ_ID = ?`;

    db.query(checkSql, [projId], (err, out) => {
      ...
      if (hierarchyExists) {
        ...
        db.query(getSql, [weekId, empId, orgId], (err2, result) => {
          ...
          db.query(nextSql, [projId, projId, approverId], (err3, next) => {
            ...
            db.query(finalSql, [projId], (err4, finalOut) => {
