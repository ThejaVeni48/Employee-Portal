...
const empProfile = (req, res) => {
 ...
 SELECT
  ...
  CASE WHEN COUNT(T.TC_MASTER_ID) > 0 THEN 1 ELSE 0 END AS TIMESHEET_EXIST
...
LEFT JOIN TC_ACCESS_CONTROLS AC
  ON U.EMP_ID = AC.EMP_ID
  AND U.ORG_ID = AC.ORG_ID
  AND AC.STATUS = 'A'

LEFT JOIN TC_SHIFT_ASSIGNMENT SA
  ON U.EMP_ID = SA.EMP_ID
  AND U.ORG_ID = SA.ORG_ID
  AND SA.STATUS = 'A'

LEFT JOIN TC_EMPLOYEE_MANAGER_MAP MAP
  ON U.EMP_ID = MAP.EMP_ID
  AND U.ORG_ID = MAP.ORG_ID
  AND MAP.STATUS = 'A'

LEFT JOIN TC_USERS M
  ON MAP.MANAGER_ID = M.EMP_ID
  AND MAP.ORG_ID = M.ORG_ID
