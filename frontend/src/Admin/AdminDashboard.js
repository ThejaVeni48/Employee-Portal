 ...
 useEffect(() => {
  ...
  const today = moment().format('YYYY-MM-DD');
     ...
     const getApproveAccess = async () => {
      try {
        const data = await fetch(`http://localhost:3001/api/showApproveAccess?companyId=${companyId}&empId=${empId}&currentDate=${today}`);
  ...
  const menuItems = [
  { section: "Dashboard", name: "Dashboard", icon: <MdDashboard />, path: "dashboard" },
  { section: "Organization", name: "Employees", icon: <MdPeople />, path: "employees" },
  { section: "Organization", name: "Departments", icon: <MdPeople />, path: "departments" },
  {
  ...
  { section: "Organization", name: "Leaves", icon: <MdSettings />, path: "adminleaves" },
  { section: "Organization", name: "Settings", icon: <MdSettings />, path: "settings" },
  { section: "Organization", name: "Change Password", icon: <MdSettings />, path: "changePassword" },
];
                      ...
                      {item.isDropdown && openMenu === item.name && (
                        <ul className="dropdownMenu">
                          {item.subItems.map((sub, sIdx) => (
                            <li
                              key={sIdx}
                              className={isActive(sub.path) ? "activeSubMenu" : ""}
onClick={() =>
  navigate(`/adminDashboard/${sub.path}`, { state: { projectsId } })
}
                            >
