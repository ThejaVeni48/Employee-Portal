...
const ProjectAssignees =  ({ project = {},approvals =[],projectEmpList = [] }) => {
...
const [refreshEmp, setRefreshEmp] = useState(false);
  ...
  const filteredEmployees = projectEmpList.filter((emp) => {
  const matchesSearch =
...
<div className="flex items-center gap-3 rounded-lg p-2">
 ...
 />
  ...
  <AddAssignees project={project} onSuccess ={()=>setRefreshEmp(prev => !prev)} approvals ={approvals}/>
        ...
        <div className="flex items-center gap-4">
          ...
          <span
            className={`h-2.5 w-2.5 rounded-full ${
              emp.STATUS === "A" ? "bg-green-500" : "bg-gray-400"
            }`}
          />
        </div>
