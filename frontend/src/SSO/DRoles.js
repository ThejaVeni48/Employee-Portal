...
const DRoles = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/api/getDefaultRoles`)
      .then((res) => res.json())
  ...
  const handleCreateRole = async () => {
    ...
    const res = await fetch("http://localhost:3001/api/createDefaultRoles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roleName,description,userId,roleCode,newStatus }),
    });
  ...
  try {
    const res = await fetch("http://localhost:3001/api/uploadRoles", {
      method: "POST",
      body: formData,
    });
