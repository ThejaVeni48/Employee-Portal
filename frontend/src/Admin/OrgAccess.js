...
// const  = (req,res)=>{
  ...
  useEffect(() => {
   fetch(`http://localhost:3001/api/getOrgAccess?companyId=${companyId}`)
      .then((res) => res.json())
  ...
  const handleCreateRole = async () => {
    ...
    const res = await fetch("http://localhost:3001/api/createOrgAccess", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ roleName,description,companyId,roleCode,newStatus,email }),
    });
