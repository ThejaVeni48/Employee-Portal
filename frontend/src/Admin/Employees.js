...
const accessCode = useSelector((state) => state.user.accessCode);
  ...
  const styles = {
    container: { minHeight: "100vh", fontFamily: "'Inter', sans-serif",padding:' 0 10px' },
    ...
    cellStyle: {
        textAlign: "left", fontSize: "12px", borderBottom: "1px solid #e0e0e0" },
    dialogFooter: {
      display: "flex",
    ...
    if (searchQuery) {
      const filtered = timesheets.filter(
        (item) =>
          ...
          item.EMP_ID?.toString().toLowerCase().includes(searchQuery.toLowerCase()) ||
          item.ROLE_NAME?.toString().toLowerCase().includes(searchQuery.toLowerCase()) 
      );
  ...
  const actionTemplate = (rowData) => (
    ...
    <button
    onClick={()=>navigate('/adminDashboard/profile', {state:{empId:rowData.EMP_ID}})}
      style={{
        border: "none",
        outline: "none",
...
const handleFileChange = async (e) => {
  ...
  try {
    ...
    // Format hireDate to YYYY-MM-DD
    rows.forEach((r) => {
      if (r.hireDate) r.hireDate = new Date(r.hireDate).toISOString().split("T")[0];
    });
...
/>
        ...
        >
          <Column field="EMP_ID" header="Employee ID" headerStyle={styles.headerStyle} bodyStyle={styles.cellStyle} />
          <Column field="DISPLAY_NAME" header="Employee Name" headerStyle={styles.headerStyle} bodyStyle={styles.cellStyle} />
          <Column field="EMAIL" header="Mail" headerStyle={styles.headerStyle} bodyStyle={styles.cellStyle} />
          <Column field="STATUS" header="Status" headerStyle={styles.headerStyle} bodyStyle={styles.cellStyle} />
          <Column header="Action" body={actionTemplate} headerStyle={styles.headerStyle} bodyStyle={styles.cellStyle} />
